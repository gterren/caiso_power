...
for idx_tr_, idx_ts_ in KFold(n_splits     = N_kfolds,
    ...
    if DL != 3:
        Y_dl_val_ts_hat_ = _joint_prob_prediction(models_, dl_scaler_, X_dl_val_ts_stnd_, Y_dl_val_ts_, W_hat_, calibration_val_ts_[i_fold, ...], g_dl_, RC, DL, y_dl_stnd, N_scenarios = 10)
    else:
        Y_dl_val_ts_hat_ = _multitask_joint_prediction(models_, dl_scaler_, X_dl_val_ts_stnd_, Y_dl_val_ts_, W_hat_, calibration_val_ts_[i_fold, ...], g_dl_, RC, DL, y_dl_stnd, N_scenarios = 10)

    # Calibrate shape of the scenarios
    smoothing_val_ts_[i_fold] = _calibrate_scenarios_temporal_structure_fit(Y_dl_val_ts_, Y_dl_val_ts_hat_, params_ = np.linspace(.2, 5., 10),
                                                                                                            score   = 'VS',
                                                                                                            verbose = False)
