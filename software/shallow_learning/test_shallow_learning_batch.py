...
for i_exp in i_exps_:
    ...
    smoothing_ts_, calibration_ts_ = _load_dict(path_to_mdl, file_name = '{}-{}-{}_e{}.pkl'.format(resource, sl_method, dl_method, i_exp))
    ...
    S2_dl_ts_hat_, C_dl_ts_hat_ = _calibrate_predictive_covariance(M_dl_ts_hat_, C_dl_ts_hat_, calibration_ts_)

    # Joint probabilistic predictions
    t_prob_ts = time.time()
    if DL != 3:
        Y_dl_ts_hat_ = _joint_prob_prediction(models_, dl_scaler_, X_dl_ts_stnd_, Y_dl_ts_, W_hat_, calibration_ts_, g_dl_, RC, DL, y_dl_stnd, N_scenarios = 100)
    else:
        Y_dl_ts_hat_ = _multitask_joint_prediction(models_, dl_scaler_, X_dl_ts_stnd_, Y_dl_ts_, W_hat_, calibration_ts_, g_dl_, RC, DL, y_dl_stnd, N_scenarios = 100)
    t_prob_ts = time.time() - t_prob_ts

    # Update scenarios shape
    Y_dl_ts_hat_ = _calibrate_scenarios_temporal_structure(Y_dl_ts_hat_, sigmas_ = smoothing_ts_)
