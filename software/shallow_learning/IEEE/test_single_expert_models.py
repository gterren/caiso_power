...
sparse_model  = int(sys.argv[1]) # 3 Different options
fc_model      = int(sys.argv[2]) # 5 Different options

pixels_selection = int(sys.argv[3]) # 5 Different options
covarites_set    = int(sys.argv[4]) # 3 Different options
chain_model      = int(sys.argv[5]) # 2 Different options

sparse_param = int(sys.argv[6]) # 7 Different options
fc_param     = int(sys.argv[7]) # 7 Different options
...
def _expert_models(X_tr_, X_ts_, Y_tr_, Y_ts_, param, chain_model, model, N_params = 7):
    ...
    S2_ts_hat_ = np.zeros((Y_ts_.shape))
    ...
    for i in range(N_hours):
        ...
        if model == 2:
            ...
            # Predictor chain for testing
            y_ts_hat_chain_, s2_ts_hat_chain_ = _ARD.predict(X_ts_p_chain_, return_std = True)
            Y_ts_hat_chain_.append(y_ts_hat_chain_)
            S2_ts_hat_chain_.append(s2_ts_hat_chain_)
        ...
        if model == 3:
            ...
            y_ts_hat_chain_, s2_ts_hat_chain_ = _BLR.predict(X_ts_p_chain_, return_std = True)
            Y_ts_hat_chain_.append(y_ts_hat_chain_)
            S2_ts_hat_chain_.append(s2_ts_hat_chain_)
        ...
        if model == 4:
            ...
            # Predictor chain for testing
            y_ts_hat_chain_, s2_ts_hat_chain_ = _GPR.predict(X_ts_p_chain_, return_std = True)
            Y_ts_hat_chain_.append(y_ts_hat_chain_)
            S2_ts_hat_chain_.append(s2_ts_hat_chain_)
        # Gather predictions
        Y_ts_hat_[:, i]  = _y_scaler.inverse_transform(Y_ts_hat_chain_[-1][:, np.newaxis])[:, 0]
        S2_ts_hat_[:, i] = _y_scaler.inverse_transform(S2_ts_hat_chain_[-1][:, np.newaxis])[:, 0] - _y_scaler.mean_
    return Y_ts_hat_, S2_ts_hat_
...
key = '{}-{}_{}-{}-{}_{}-{}'.format(sparse_model, fc_model, pixels_selection, covarites_set, chain_model, sparse_param, fc_param)
print(key)
...
Y_ts_hat_, S2_ts_hat_ = _expert_models(X_tr_, X_ts_, Y_tr_, Y_ts_, fc_param, chain_model, fc_model)
...
with open(path_to_rst + r"TS_ME_pred_{}-{}.pkl".format(i_resource, i_asset), 'wb') as handle:
    pickle.dump([Y_ts_, Y_ts_hat_, S2_ts_hat_, Z_p_[:, -N_day_samples_ts:, :]], handle, protocol = pickle.HIGHEST_PROTOCOL)
