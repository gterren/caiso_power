...
for i_exp in split_experiments_into_jobs_per_batches(exps_, i_batch, N_batches, i_job, N_jobs)[i_job]:
    ...
    for i_theta in range(len(thetas_)):
        ...
        for idx_tr_, idx_ts_ in KFold(n_splits     = N_kfolds,
            ...
            W_hat_ = np.ones((X_sl_val_ts_stnd_[:, g_sl_ != g_sl_[-1]].shape[1], Y_sl_val_ts_.shape[1]))
    ...
    if SL != 4:
        ...
        _save_test_in_csv_file(E_sl_, key, t_end, i_theta_.tolist(), [thetas_[i_theta] for i_theta in i_theta_], i_resource, path_to_rst, 'SLDetTest.csv')
